...
var
  ...
  Server: TWarenflussRestServer;

begin
  Writeln('================================================================');
  Writeln('  WARENFLUSS - ENTERPRISE REST API & SOA SERVER');
  Writeln('================================================================');
  ...
  Server := TWarenflussRestServer.Create('8088', AuthSvc, ProdSvc, InvSvc, XferSvc, SalesSvc, PurchSvc, ReportSvc);
  try
    Server.Start;
    Writeln('REST API listening on http://localhost:8088/api');
    Writeln('Endpoints available:');
    Writeln('  POST /api/auth/login');
    Writeln('  GET  /api/products');
    Writeln('  GET  /api/inventory/stock');
    Writeln('  GET  /api/reports/dashboard');
    Writeln('');
